...
test('phase-24 baseline: package.json is at 1.24.2', () => {
  const pkg = JSON.parse(fs.readFileSync(path.join(REPO_ROOT, 'package.json'), 'utf8'));
  assert.equal(pkg.version, '1.24.2');
});

test('phase-24 baseline: plugin.json is at 1.24.2', () => {
  const plugin = JSON.parse(fs.readFileSync(path.join(REPO_ROOT, '.claude-plugin', 'plugin.json'), 'utf8'));
  assert.equal(plugin.version, '1.24.2');
});

test('phase-24 baseline: marketplace.json is at 1.24.2 in both slots', () => {
  const market = JSON.parse(fs.readFileSync(path.join(REPO_ROOT, '.claude-plugin', 'marketplace.json'), 'utf8'));
  assert.equal(market.metadata.version, '1.24.2');
  assert.equal(market.plugins[0].version, '1.24.2');
});
