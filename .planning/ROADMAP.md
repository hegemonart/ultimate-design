...
**Requirements**: REF-06 through REF-14 (to be defined during plan-phase)
...
**Success Criteria** (what must be TRUE):
 ...
 10. **`reference/framer-motion-patterns.md`** — implementation companion to `reference/motion.md` (philosophy/when). Covers: `motion.div` / `motion.span` basics; spring vs. tween configuration (stiffness, damping, mass); `AnimatePresence` for exit animations (mode: wait/sync/popLayout); layout animations (`layoutId` for shared element transitions); variants + orchestration (`staggerChildren`, `delayChildren`); gesture-driven motion (`whileHover`, `whileTap`, `drag`); scroll-linked animations (`useScroll`, `useTransform`); `prefers-reduced-motion` compliance pattern (check `useReducedMotion()` hook, fallback to instant transitions); 60fps constraint — only `transform` and `opacity` should animate (no layout-thrashing properties); performance: `will-change` hygiene, `MotionConfig` for global reduced-motion toggle. Wired into `design-executor` (type:motion) and `design-auditor` (motion anti-pattern checks: non-transform animations, missing reduced-motion guard, keyboard-action animations per motion.md Q1).
 11. **Agent integration** — `design-context-builder` reads brand-voice for archetype resolution; `design-auditor` reads iconography + performance + gestalt + framer-motion anti-patterns for audit pillars; `design-executor` reads iconography (type:icons), performance (type:layout + type:performance), brand-voice (type:copy), framer-motion-patterns (type:motion); `design-pattern-mapper` extended with `iconography` and `brand-voice` categories.
 12. README / plugin.json / marketplace.json updated; CHANGELOG v1.15.0 entry lists the 8 new references + impeccable removal + salvage archive; regression baseline locked at `test-fixture/baselines/phase-15/`.

**Plans**: 5 plans across 2 waves

**Wave A — salvage + purge + authored references (4 plans):**
...
- [ ] 15-04-PLAN.md — Author `reference/framer-motion-patterns.md` (implementation companion to `reference/motion.md`): spring/tween config, AnimatePresence, layout animations, variants+orchestration, gesture-driven, scroll-linked, `useReducedMotion()` compliance pattern, 60fps constraint, MotionConfig global toggle. Wire into `design-executor` (type:motion required-reading), `design-auditor` (motion anti-pattern checks: non-transform animations, missing reduced-motion guard, keyboard-action animations). Verify stage gains two new checks: `MOTION-A11Y` (prefers-reduced-motion guard present) and `MOTION-PERF` (no layout-thrashing animated properties) (REF-14)

**Wave B — closeout (1 plan):**
- [ ] 15-05-PLAN.md — **Phase closeout**: pipeline smoke on `test-fixture/` confirming agents actually reach the new references (log lines appear in task-NN.md references block); refresh README.md (knowledge expansion section, impeccable migration notice, motion patterns section); refresh plugin.json + marketplace.json (version, description, keywords — add "iconography", "brand-voice", "performance-budget", "framer-motion", "motion-design"); append CHANGELOG.md v1.15.0 entry; lock regression baseline at `test-fixture/baselines/phase-15/` (MAN-13, MAN-14)
...
**Success Criteria** (what must be TRUE):
  ...
  6. README / plugin.json / marketplace.json updated; CHANGELOG v1.18.0 entry; regression baseline locked at `test-fixture/baselines/phase-18/`.
...
**Wave B — closeout (1 plan):**
- [ ] 18-05-PLAN.md — **Phase closeout**: refresh README.md (advanced-craft references section); refresh plugin.json + marketplace.json (version, description, keywords — add "variable-fonts", "container-queries", "view-transitions"); append CHANGELOG.md v1.18.0 entry; lock regression baseline at `test-fixture/baselines/phase-18/` (MAN-19, MAN-20)
...
**Success Criteria** (what must be TRUE):
  ...
  11. README / plugin.json / marketplace.json updated; CHANGELOG v1.20.0 entry; regression baseline at `test-fixture/baselines/phase-20/`.
...
- [ ] 20-14-PLAN.md — **Phase closeout**: race-condition test (4 concurrent agents on STATE.md), regression baseline at `test-fixture/baselines/phase-20/`, README / plugin.json / marketplace.json refresh, CHANGELOG v1.20.0 entry (SDK-12)
...
**Wave D — closeout (1 plan):**
- [ ] 21-12-PLAN.md — **Phase closeout**: README + plugin.json + marketplace.json refresh, CHANGELOG v1.21.0, regression baseline at `test-fixture/baselines/phase-21/`
...
**Wave C — closeout (1 plan):**
- [ ] 22-12-PLAN.md — **Phase closeout**: README + plugin.json + marketplace.json refresh, CHANGELOG v1.22.0, regression baseline at `test-fixture/baselines/phase-22/` (SDK-36)
...
**Wave D — closeout (1 plan):**
- [ ] 23-16-PLAN.md — **Phase closeout**: final regression baseline at `test-fixture/baselines/phase-23/`, README + plugin.json + marketplace.json refresh (SDK milestone complete note, full programmatic surface), CHANGELOG v1.23.0 entry. **SDK milestone complete.**
...
**Version sequence** (MAJOR.MINOR.PATCH = milestone.phase.sub-phase): v1.13.3 is current. Historical npm tags: v1.0.0–v1.0.7 (Phases 6–13, pre-scheme). v1.10.1 (Phase 10.1) → v1.13.0 (Phase 13) → v1.13.1 (Phase 13.1) → v1.13.2 (Phase 13.2) → v1.13.3 (Phase 13.3) → v1.14.0 (Phase 14 — AI-Native Design Tool Connections) → v1.15.0 (Phase 15 — Foundational References + Impeccable Removal) → v1.16.0 (Phase 16 — Component Benchmark Corpus Waves 1–2) → v1.17.0 (Phase 17 — Corpus Waves 3–5 + Pipeline Integration) → v1.18.0 (Phase 18 — Advanced Craft References) → v1.19.0 (Phase 19 — Platform + Inclusive + UX Research References) → v1.20.0 (Phase 20 — SDK Foundation) → v1.21.0 (Phase 21 — SDK Headless) → v1.22.0 (Phase 22 — SDK Observability) → v1.23.0 (Phase 23 — SDK Domain Primitives; SDK milestone complete).
...
| **10.1. Optimization Layer + Cost Governance (INSERTED)** — router, cache manager, budget enforcement, telemetry, model tiers, lazy spawning, /gdd:optimize | 4/6 (5 feature + 1 closeout) | In Progress | v1.10.1 | - |
...
| 14. AI-Native Design Tool Connections — paper.design + pencil.dev + unified canvas interface | 0/6 (5 plans + 1 closeout across 3 waves) | Planned | v1.14.0 | - |
| 15. Design Knowledge Expansion — 7 foundational references (iconography, performance, brand-voice, visual-hierarchy-layout, design-system-guidance, gestalt, design-systems-catalog) + impeccable removal + salvage | 0/4 (3 feature + 1 closeout across 2 waves) | Planned | v1.15.0 | - |
| 16. Component Benchmark Corpus — harvester + synthesizer agents + `/gdd:benchmark` skill + Waves 1–2 (15 specs: Button/Input/Select/Checkbox/Radio/Switch/Link/Label + Card/Modal/Drawer/Popover/Tooltip/Accordion/Tabs) | 0/5 (4 feature + 1 closeout across 3 waves) | Planned | v1.16.0 | - |
| 17. Component Benchmark Corpus — Waves 3–5 (20 specs: feedback + nav/data + advanced) + pipeline integration (auditor conformance scoring, executor pre-flight, doc-writer scaffold, pattern-mapper convergence) | 0/5 (4 feature + 1 closeout across 3 waves) | Planned | v1.17.0 | - |
| 18. Advanced Craft References — variable-fonts-loading, image-optimization, css-grid-layout (grids/container queries/fluid typography/safe areas), motion-advanced (spring/stagger/scroll-driven/FLIP/View Transitions) | 0/5 (4 feature + 1 closeout across 2 waves) | Planned | v1.18.0 | - |
| 19. Platform, Inclusive & UX Research References — platforms (iOS/Android/web/visionOS), rtl-cjk-cultural, onboarding-progressive-disclosure, user-research, information-architecture, form-patterns, data-visualization | 0/8 (7 feature + 1 closeout across 3 waves) | Planned | v1.19.0 | - |
| 20. GDD SDK Foundation — `gdd-state` Node module + lockfiles + atomic RMW + 4 transition gates + MCP server (11 typed tools) + event stream foundation + prompt-sanitizer + GDDError taxonomy + migrate 5 stage skills + utility skills + hooks | 0/14 (13 feature + 1 closeout across 4 waves) | Planned | v1.20.0 | - |
| 21. GDD SDK Headless — session-runner + context-engine + tool-scoping + logger + pipeline-runner state machine + parallel research bootstrap + parallel mappers/discussants + gdd-sdk CLI + cross-harness MCP (Codex/Gemini) + E2E headless integration test | 0/12 (11 feature + 1 closeout across 4 waves) | Planned | v1.21.0 | - |
| 22. GDD SDK Observability — typed event schema expansion (~20 event types) + CLI transport (JSONL with filter language) + WebSocket transport + telemetry reader/aggregator + reflector pipeline ported to events + authority-watcher + update-checker + injection-scanner + cache-warmer + budget-enforcer + gdd-router as primitives + connection probes with retry/fallback | 0/12 (11 feature + 1 closeout across 3 waves) | Planned | v1.22.0 | - |
| 23. GDD SDK Domain Primitives — knowledge layer SDK (intel + graph + skill manifest + cycle/workstream) + visual engine (image diff + screenshots + baselines) + design-token engine (multi-source + diff + apply) + Touches: analyzer + parallelism decision engine + audit aggregator + budget allocator + reference resolver + pause/resume serializer + NNG/anti-pattern/WCAG checkers + handoff parser + spec↔code↔visual triangulation + git ops + distribution as `@hegemonart/gdd-sdk` + migration/headless/API docs | 0/16 (15 feature + 1 closeout across 4 waves) | Planned | v1.23.0 | - |
