...
**Depends on**: Phase 10 (consumes `.design/intel/`, `.design/learnings/`, agent-metrics data). Without Phase 10, reflection has nothing to reflect on.
**Requirements**: SLF-01 through SLF-08 (to be defined during plan-phase)
**Success Criteria** (what must be TRUE):
  1. `design-reflector` agent runs at cycle completion (hooked into `/gdd:audit`). Reads `.design/intel/`, `.design/learnings/`, STATE.md history, agent metrics, and DESIGN-VERIFICATION.md. Produces `.design/reflections/<cycle-slug>.md` with: what surprised us, which decisions recurred, which agents over/under-performed, which anti-patterns appeared N times, which discussant questions got low-value answers.
  2. `.design/agent-metrics.json` tracks per-agent actual duration, gap-rate (verify gaps attributable), deviation-rate (deviations per cycle), context-cost (tokens consumed per spawn). Updated incrementally after each agent run.
  3. **Frontmatter feedback loop**: reflector proposes updates to agent frontmatter (`typical-duration-seconds` from measured data, `parallel-safe` downgrades if conflicts seen, `reads-only: false` if write patterns detected). User reviews via `/gdd:apply-reflections` — diff + confirm.
  4. **Reference-update proposer**: when N≥3 cycles flag the same missing anti-pattern, same missing heuristic, or same reference gap, reflector drafts an addition to `reference/anti-patterns.md` / `reference/heuristics.md` / a new reference file. User reviews and applies via `/gdd:apply-reflections`.
  5. **Discussant question-quality feedback**: discussant logs which questions got "don't know / doesn't matter / not relevant" answers. After N cycles, reflector proposes pruning or rewording low-value questions. User reviews.
  6. **Global skills layer**: `~/.claude/gdd/global-skills/` — cross-project codified decisions (e.g., "I always use oklch colors"). Loaded alongside project-local `./.claude/skills/` via CLAUDE.md routing. Populated from reflections that user promotes from project-local to global.
  7. `/gdd:reflect` runs the reflector on demand (not just at cycle completion). Useful for mid-cycle retrospectives or when Investigating "why did the last verify fail?"
...
- [ ] 11-02-PLAN.md — `.design/agent-metrics.json` tracker; per-agent duration/gap/deviation/context-cost instrumentation; frontmatter feedback-loop proposer (SLF-03, SLF-04)
...
*Infrastructure (Wave A):*
  ...
  4. `test-fixture/baselines/phase-<N>/` — regression baselines for Phases 7–11 captured as golden files; Phase 12 parses and compares.
...
- [x] 12-02-PLAN.md — Regression baseline capture: lock Phase 7–11 outputs to `test-fixture/baselines/phase-<N>/`; baseline-diff test harness; baseline re-lock workflow with explicit user confirmation (TST-04, TST-05)
...
- [ ] 12-06-PLAN.md — Feature correctness: sketch-determinism, connection-probe (all 8 connections mocked), figma-writer-dry-run, reflection-proposal, deprecation-redirect, NNG coverage, injection-scanner (TST-25 through TST-32)
...
- **Token-usage tracking (optional)**: track how many tokens a baseline pipeline run consumes; alert on regression. Defer to Phase 11 (self-improvement) — needs reflector infrastructure.
...
### Design constraints for Phase 7 + 8 — honor these to avoid rework in Phase 9/10/11
...
2. **Agent frontmatter is extensible**. Mark `typical-duration-seconds` as "initial estimate, may be augmented." Phase 11's reflector adds `measured-duration-seconds` without replacing. Document the extension path in `agents/README.md`.
...
4. **Cycle completion writes a standard SUMMARY artifact** at `.design/cycles/<slug>/CYCLE-SUMMARY.md` with fixed sections: decisions, deviations, metrics, gaps, outcome. Phase 10 parses it; Phase 11 reflects on it. Agreeing on the shape NOW avoids later archeology.
5. **STATE.md schema versioned via `pipeline_state_version`** — commit to semver + migration notes when Phase 10/11 extend. Prevents silent breakage.
...
11. **Version-bump + release cadence per phase** — sequential patch bumps, no version jumping: Phase 6 shipped v1.0.0. Phase 7 → v1.0.1. Phase 8 → v1.0.2. Phase 9 → v1.0.3. Phase 10 → v1.0.4. Phase 11 → v1.0.5. Phase 12 → v1.0.6. Version bump is a task in each phase's final plan (not an afterthought). Breaking changes (like plan 07-00's pipeline reshape) are documented in DEPRECATIONS.md and release notes — versioning stays sequential per user directive.
...
**Execution Order:**
Phases 1 → 6 execute in numeric order. Phases 7 and 8 can run in parallel (see Parallelization section). Phase 9 depends on both 7 and 8. Phase 10 depends on Phase 8 (graphify connection). Phase 11 depends on Phase 10 (memory infrastructure — reflection without memory is ritual). Phase 12 depends on all prior phases (tests validate their features; regression baselines lock from here forward). Phase 13 depends on Phase 12 (CI/CD orchestrates tests) — sequential, not parallel.

**Version sequence** (sequential patch bumps per user directive — no version jumps even on breaking changes): v1.0.0 (Phase 6) → v1.0.1 (Phase 7) → v1.0.2 (Phase 8) → v1.0.3 (Phase 9) → v1.0.4 (Phase 10) → v1.0.5 (Phase 11) → v1.0.6 (Phase 12) → v1.0.7 (Phase 13).
...
| 11. Self-Improvement — reflector, metrics, reference-update proposer, global skills | 0/5 (4 feature + 1 closeout) | Planned | v1.0.5 | - |
| 12. Test Coverage — ported GSD patterns + gdd-unique tests + basic CI + regression baselines | 1/7 (6 plans + 1 closeout across 3 waves) | 2/7 | In Progress|  |
...
| `/gsd-forensics` | Phase 11 | Post-mortem investigation for failed flows — pairs with reflector, not useful without learnings data | `/gdd:forensics` reads cycle history + agent metrics to diagnose "what went wrong" |
...
| `/gsd-session-report` | Phase 11 | Token usage + work summary + outcomes — pairs with reflector agent | `/gdd:session-report` at cycle end feeds reflection data |
